...
public class Slot : MonoBehaviour, IDropHandler
{
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        {
            print(eventData.pointerDrag.GetComponent<DragAndDrop>().lastSlot);
            if(!slotUsed)
            {
                eventData.pointerDrag.GetComponent<DragAndDrop>().lastSlot.GetComponent<Slot>().slotUsed = false;

                if (eventData.pointerDrag.GetComponent<DragAndDrop>().equipedItem && eventData.pointerDrag.GetComponent<DragAndDrop>().lastSlot != this.gameObject)
                {
